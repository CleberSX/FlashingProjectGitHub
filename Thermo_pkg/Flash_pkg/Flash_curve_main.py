...
def calculate_vapor_liquid_equilibrium(eos_obj, michelsen_obj, flash_obj, prop_obj, pressure, temperature, 
    ...
    _is_stable, K_michelsen = michelsen_obj(eos_obj, P, T, z, initial_K_values, max_iter, tolerance)
    ...
    if 0.0 <= F_V_flash <= 1.0:
        result, _infodict, ier, mesg = fsolve(func=flash_obj.flash_residual_function, x0=Vector_ToBe_Optimized, 
        ...
        _f_L, _Z_L = eos_obj.calculate_fugacities_with_minimum_gibbs_energy(P, T, x_i, 'liquid')
        _f_V, _Z_V = eos_obj.calculate_fugacities_with_minimum_gibbs_energy(P, T, y_i, 'vapor')
...
def calculate_molar_fraction_curve(pressure, temperature_init, temperature_max, molar_mass, global_molar_fractions,
    ...
    for index, T in enumerate(temperature_points):
        F_V, _K, x_i, y_i, rho_V, rho_L = calculate_vapor_liquid_equilibrium(eos_obj, michelsen_obj, flash_obj,
