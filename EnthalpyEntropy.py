...
'''
class HSFv:
      '''THIS CLASS CALCULATE ENTHALPY AND ENTROPY; ALSO DETERMINE QUALITY WHEN TWO-PHASE OCCURS'''
      def __init__(self, pC, TR, Tc, AcF, Cp, MM, hR, sR, printResults=True):
            self.pC, self.TR, self.Tc = pC, TR, Tc
            self.AcF, self.Cp, self.MM, self.hR, self.sR = AcF, Cp, MM, hR, sR
            self.printResults = printResults

      def __call__(self, p, pR, T, z):
...
=========================================================================================================
executing ...  
1st: creating obj -- instantiating the class
2nd: executing bubble_obj to obtain TR
3rd: finaly executing the object for the specific variables (p, pR, T, z)
=========================================================================================================
'''
hsfv_obj = HSFv(pC, TR, Tc, AcF, Cp, MM, hR, sR)
pR, y_sat, Sy_sat, counter = bubble_obj(TR, z)
F_V, h, s = hsfv_obj(p, pR, T, z)
